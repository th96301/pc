...
string d[64][3];
int debug = 0;
bool saveDataToFile(const string s[64][3], const string& filename) {
...
bool readDataFromFile(string s[64][3], const string& filename) {
	std::ifstream inFile(filename);

	if (!inFile.is_open()) {
		cerr << "无法打开文件 " << filename << " 进行读取！" << std::endl;
		return false;
	}
	...
	try {
		...
		while (getline(inFile, line) && row < 63) {
			...
			// 使用制表符分隔每一行的内容
			while ((end = line.find('\t', start)) != string::npos && col < 2) {
				s[row][col] = line.substr(start, end - start);
				start = end + 1;
				col++;
			}

			// 获取最后一个字段（或者当没有制表符时获取整行）
			if (col < 3) {
				s[row][col] = line.substr(start);
			}
		...
		cerr << "读取文件时发生错误: " << e.what() << std::endl;
		return false;
	}
}
int programs() {
...
int settings() {
	string t;
	cout << "输入序号" << endl << "0.返回 1.切换主题 2.重置文档" << endl;
	while (cin >> t) {
		if (t == "0") {
			return 0;
		} else if (t == "1") {
			...
			string tt;
			cout << "此操作不可撤销,是否继续(Y/N)" << endl;
			cin >> tt;
			if (tt == "Y" || tt == "y" || tt == "Yes" || tt == "yes") {
				system("cls");
				printf("\rLaoding");
				for (int i = 0; i < all; i++) {
					if (i <= 2) {
